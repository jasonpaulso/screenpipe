...
//! `store: Option<&daimonion_secrets::SecretStore>` as its first parameter.
//!
...
pub fn oauth_token_path_instance(integration_id: &str, instance: Option<&str>) -> PathBuf {
    ...
    daimonion_core::paths::default_screenpipe_data_dir().join(name)
}
...
/// `daimonion_core::connections::sync::remote_is_newer`.
fn next_refresh_gen(
...
pub async fn write_oauth_token_instance(
    ...
    // `daimonion_core::connections::sync::remote_is_newer` immune to the
    // wall-clock skew between two devices refreshing the same connection.
    {
        let gen_field = daimonion_core::connections::sync::OAUTH_REFRESH_GEN_FIELD;
        ...
        let prior_gen = prior
            ...
            .unwrap_or(0);
        let highwater =
            daimonion_core::connections::sync::read_oauth_gen_highwater(&data_dir, &manifest_key);
        ...
        daimonion_core::connections::sync::bump_oauth_gen_highwater(
            &data_dir,
            &manifest_key,
            new_gen,
        );
    ...
    if let Some(s) = store {
        ...
        daimonion_core::connections::sync::clear_connection_tombstone(
...
pub async fn delete_oauth_token_instance(
    ...
    daimonion_core::connections::sync::record_connection_tombstone(
        &daimonion_core::paths::default_screenpipe_data_dir(),
        &manifest_key,
    );
    Ok(())
