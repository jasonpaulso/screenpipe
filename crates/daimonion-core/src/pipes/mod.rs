...
fn read_chatgpt_token_from_secrets() -> Option<String> {
    use daimonion_secrets::keychain::{get_key, KeyResult};
    ...
    let result = tokio::task::block_in_place(|| {
        tokio::runtime::Handle::current().block_on(async {
            ...
            let store = daimonion_secrets::SecretStore::new(pool, secret_key)
                .await
                .ok()?;
...
fn read_store_bin(path: &Path) -> Option<serde_json::Value> {
    ...
    if data.len() >= STORE_MAGIC.len() && &data[..STORE_MAGIC.len()] == STORE_MAGIC {
        #[cfg(feature = "secrets")]
        {
            ...
            let plaintext =
                daimonion_vault::crypto::decrypt_small(&data[STORE_MAGIC.len()..], &key).ok()?;
...
impl PipeManager {
    ...
    pub async fn start_scheduler(&mut self) -> Result<()> {
        ...
        let handle = tokio::spawn(async move {
            ...
            let mut workflow_rx =
                daimonion_events::subscribe_to_event::<serde_json::Value>("workflow_event");
            // pipe_completed:* uses subscribe_to_all with prefix filtering below
            let mut pipe_completed_rx = daimonion_events::subscribe_to_all_events();
                ...
                for (name, config, body) in &pipe_snapshot {
                    ...
                    tokio::spawn(async move {
                        ...
                        let _ = daimonion_events::send_event(
                            &event_name,
                            daimonion_events::PipeCompletedEvent {
