...
use daimonion_db::CustomOcrConfig as DBCustomOcrConfig;
...
impl From<OcrEngine> for daimonion_db::OcrEngine {
    fn from(val: OcrEngine) -> Self {
        match val {
            OcrEngine::Unstructured => daimonion_db::OcrEngine::Unstructured,
            OcrEngine::Tesseract => daimonion_db::OcrEngine::Tesseract,
            OcrEngine::WindowsNative => daimonion_db::OcrEngine::WindowsNative,
            OcrEngine::AppleNative => daimonion_db::OcrEngine::AppleNative,
            OcrEngine::Custom(config) => {
                daimonion_db::OcrEngine::Custom(DBCustomOcrConfig::from(config))
            }
        }
    }
}

impl From<daimonion_db::OcrEngine> for OcrEngine {
    fn from(engine: daimonion_db::OcrEngine) -> Self {
        match engine {
            daimonion_db::OcrEngine::Unstructured => OcrEngine::Unstructured,
            daimonion_db::OcrEngine::Tesseract => OcrEngine::Tesseract,
            daimonion_db::OcrEngine::WindowsNative => OcrEngine::WindowsNative,
            daimonion_db::OcrEngine::AppleNative => OcrEngine::AppleNative,
            daimonion_db::OcrEngine::Custom(config) => OcrEngine::Custom(config.into()),
        }
    }
}
