...
use daimonion_audio::transcription::engine::TranscriptionEngine;
use daimonion_audio::transcription::whisper::model::{
    download_whisper_model, get_cached_whisper_model_path,
};
use daimonion_audio_eval::{load_utterances, score_transcription, LibriUtterance};
use serde::Serialize;
...
async fn run_model(
    ...
    for (idx, utt) in utterances.iter().take(take).enumerate() {
        let (samples, source_rate) = daimonion_audio::pcm_decode(&utt.flac_path)
            .with_context(|| format!("pcm_decode {}", utt.flac_path.display()))?;
        let samples_16k = if source_rate != 16_000 {
            daimonion_audio::resample(&samples, source_rate, 16_000)?
        } else {
            samples
        };
        ...
        let wer = daimonion_audio_eval::word_error_rate(&utt.reference_transcript, &hyp);
        let cer = daimonion_audio_eval::char_error_rate(&utt.reference_transcript, &hyp);
        eprintln!(
            "[{}][{}/{}] {} ({:.2}s) wer={:.3} cer={:.3}",
            spec.name,
            idx + 1,
