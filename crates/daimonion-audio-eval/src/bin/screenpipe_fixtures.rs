...
use daimonion_audio_eval::{load_utterances, LibriUtterance, RttmSegment};
use serde::Serialize;
...
fn decode_clip(utt: &LibriUtterance, max_seconds: Option<f64>) -> Result<Clip> {
    let (raw, source_rate) = daimonion_audio::pcm_decode(&utt.flac_path)
        .with_context(|| format!("decode {}", utt.flac_path.display()))?;
    let mut samples = if source_rate != SAMPLE_RATE {
        daimonion_audio::resample(&raw, source_rate, SAMPLE_RATE)?
    } else {
        raw
    };
