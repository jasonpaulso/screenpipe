...
async fn low_tier_db_initializes_successfully() {
    let config = DbConfig::for_tier(daimonion_config::DeviceTier::Low);
...
async fn mid_tier_db_initializes_successfully() {
    let config = DbConfig::for_tier(daimonion_config::DeviceTier::Mid);
    assert_eq!(config.mmap_size, 0); // mmap disabled to prevent DB corruption
...
async fn write_pool_size_increased_for_all_tiers() {
    ...
    let mid_config = DbConfig::for_tier(daimonion_config::DeviceTier::Mid);
    assert!(
    ...
    let low_config = DbConfig::for_tier(daimonion_config::DeviceTier::Low);
    assert!(
