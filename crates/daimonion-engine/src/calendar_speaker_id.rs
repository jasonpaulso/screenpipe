...
use daimonion_db::ActiveSpeaker;
...
async fn deduplicate_speaker_by_name(
    db: &daimonion_db::DatabaseManager,
    newly_named_id: i64,
    name: &str,
) {
...
async fn deduplicate_existing_speaker_aliases(db: &daimonion_db::DatabaseManager) {
...
pub fn start_speaker_identification(
    db: Arc<daimonion_db::DatabaseManager>,
...
async fn auto_name_input_speaker(db: Arc<daimonion_db::DatabaseManager>, user_name: &str) {
...
async fn run_speaker_identification_loop(
    ...
    let mut meeting_sub =
        daimonion_events::subscribe_to_event::<MeetingEventData>("meeting_started");
    let mut ended_sub = daimonion_events::subscribe_to_event::<MeetingEventData>("meeting_ended");
...
mod tests {
    ...
    async fn setup_db() -> daimonion_db::DatabaseManager {
    ...
    async fn seed_speaker(
        ...
        use daimonion_db::{AudioDevice, DeviceType};
