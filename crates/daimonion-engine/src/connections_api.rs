...
use daimonion_connect::connections::ConnectionManager;
use daimonion_connect::oauth::{self as oauth_store, PENDING_OAUTH};
use daimonion_connect::whatsapp::WhatsAppGateway;
use daimonion_secrets::SecretStore;
...
use daimonion_connect::connections::browser::{BrowserRegistry, BrowserSummary, EvalError};
...
async fn list_connections(State(state): State<ConnectionsState>) -> Json<Value> {
    ...
    let wa_connected = matches!(
        wa_status,
        daimonion_connect::whatsapp::WhatsAppStatus::Connected { .. }
    );
    ...
    if let Some(arr) = data.as_array_mut() {
        ...
        let (ics_feed_count, ics_enabled_count, ics_error) =
            match daimonion_connect::ics_calendar::load_ics_calendar_settings_from_store(
...
#[cfg(target_os = "windows")]
fn is_native_calendar_available() -> bool {
    daimonion_connect::calendar_windows::ScreenpipeCalendar::is_available()
}
...
fn ics_feed_summaries(
    settings: &daimonion_connect::ics_calendar::IcsCalendarSettings,
) -> Vec<Value> {
    settings
        .entries
        .iter()
...
async fn ics_calendar_events(
    ...
    let settings = match daimonion_connect::ics_calendar::load_ics_calendar_settings_from_store(
        &state.screenpipe_dir,
    ) {
    ...
    let events = daimonion_connect::ics_calendar::fetch_ics_calendar_events(
        &client,
        &enabled,
...
fn resolve_auth(
    proxy_auth: &daimonion_connect::connections::ProxyAuth,
    creds: Option<&Map<String, Value>>,
    oauth_token: Option<&str>,
    oauth_extras: Option<&Value>,
) -> ResolvedAuth {
    use daimonion_connect::connections::ProxyAuth;
    match proxy_auth {
...
async fn connection_proxy(
    ...
    let creds = mgr
        ...
        .ok()
        .flatten();
    let oauth_json = daimonion_connect::oauth::load_oauth_json(
        state.secret_store.as_deref(),
        &id,
        instance_ref,
    )
    ...
    let oauth_token = daimonion_connect::oauth::get_valid_token_instance(
        state.secret_store.as_deref(),
        &http_client,
        &id,
        instance_ref,
    ...
    if matches!(auth, ResolvedAuth::None)
        && !matches!(
            proxy_cfg.auth,
            daimonion_connect::connections::ProxyAuth::None
        )
    {
        tracing::warn!(
        ...
        let has_oauth_state =
            !daimonion_connect::oauth::list_oauth_instances(state.secret_store.as_deref(), &id)
                .await
                .is_empty();
        let error = if has_oauth_state {
            let display_name = mgr.find_def(&id).map(|d| d.name).unwrap_or(id.as_str());
            daimonion_connect::oauth::describe_oauth_error(
                state.secret_store.as_deref(),
                &id,
                display_name,
                instance_ref,
...
mod tests {
    ...
    fn ics_feed_summaries_never_include_urls() {
        let settings = daimonion_connect::ics_calendar::IcsCalendarSettings {
            entries: vec![daimonion_connect::ics_calendar::IcsCalendarEntry {
                name: "Work".to_string(),
    ...
    impl daimonion_connect::connections::browser::Browser for OwnerRecordingBrowser {
        fn id(&self) -> &str {
            "owned-default"
        }
        fn name(&self) -> &str {
        ...
        async fn eval(
            ...
            daimonion_connect::connections::browser::EvalResult,
            daimonion_connect::connections::browser::EvalError,
        > {
            ...
            *self.last_owner.lock().await = None;
            Ok(daimonion_connect::connections::browser::EvalResult {
                ok: true,
