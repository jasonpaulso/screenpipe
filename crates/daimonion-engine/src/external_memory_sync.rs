...
//!
//! Mirrors the shape of `daimonion_connect::oauth_refresh_scheduler`
...
use anyhow::Result;
use daimonion_connect::connections::{load_connection, SavedConnection};
use daimonion_core::memories::external_sync::{
...
fn apply(
    ...
    let entries_used = entries
        .len()
        .min(daimonion_core::memories::external_sync::MAX_ENTRIES_PER_DIGEST);
...
fn resolve_claude_code_path(creds: &serde_json::Map<String, Value>) -> Result<PathBuf> {
    daimonion_connect::connections::claude_code::resolve_home_path(creds)
}

fn resolve_codex_path(creds: &serde_json::Map<String, Value>) -> Result<PathBuf> {
    daimonion_connect::connections::codex::resolve_home_path(creds)
}
...
mod tests {
    use super::*;
    use daimonion_core::memories::external_sync::{marker_end, marker_start};
    use serde_json::json;
