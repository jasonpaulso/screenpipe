...
use daimonion_screen::monitor::SafeMonitor;
...
mod macos {
    ...
    async fn record_one_chunk(
        ...
        use daimonion_screen::capture_screenshot_by_window::{
        ...
        let daimonion_screen::HdCapture {
            stream,
    ...
    fn start_hd_ffmpeg(out: &Path, fps: u32) -> Result<tokio::process::Child> {
        let ffmpeg = daimonion_core::find_ffmpeg_path().context("ffmpeg not found")?;
        let mut cmd = daimonion_core::ffmpeg_cmd_async(&ffmpeg);
