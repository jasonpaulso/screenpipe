...
use daimonion_audio::audio_manager::AudioManager;
use daimonion_core::sync::SyncServiceHandle;
...
pub struct AppState {
    ...
    pub audio_metrics: Arc<daimonion_audio::metrics::AudioPipelineMetrics>,
    ...
    pub pipe_permissions:
        Arc<DashMap<String, Arc<daimonion_core::pipes::permissions::PipePermissions>>>,
    ...
    pub vault: daimonion_vault::VaultManager,
    ...
    pub browser_registry: Arc<daimonion_connect::connections::browser::BrowserRegistry>,
    ...
    pub secret_store: Option<Arc<daimonion_secrets::SecretStore>>,
...
pub struct SCServer {
    ...
    pub owned_browser: Option<Arc<daimonion_connect::connections::browser::OwnedBrowser>>,
    ...
    pub oauth_refresher:
        Option<Arc<daimonion_connect::oauth_refresh_scheduler::OAuthRefreshScheduler>>,
...
impl SCServer {
    ...
    pub fn new(
        ...
        SCServer {
            ...
            video_quality,
            pipe_manager: None,
            vision_metrics: Arc::new(daimonion_screen::PipelineMetrics::new()),
            audio_metrics,
    ...
    pub async fn start(self) -> Result<(), std::io::Error> {
        ...
        if self.advertise_mdns {
            if let Err(e) = daimonion_connect::mdns::advertise(self.addr.port()) {
    ...
    pub async fn create_router(&self) -> Router {
        ...
        let app_state = Arc::new(AppState {
            ...
            vault: daimonion_vault::VaultManager::new(self.screenpipe_dir.clone()),
            manual_meeting: self
                .manual_meeting
                .clone()
                .unwrap_or_else(|| Arc::new(tokio::sync::RwLock::new(None))),
            browser_bridge: crate::routes::browser::BrowserBridge::new(),
            browser_registry: daimonion_connect::connections::browser::BrowserRegistry::new(),
        ...
        // OwnedWebviewHandle.
        {
            use daimonion_connect::connections::browser::UserBrowser;
        ...
        let cm: crate::connections_api::SharedConnectionManager = Arc::new(Mutex::new(
            daimonion_connect::connections::ConnectionManager::new(
                self.screenpipe_dir.clone(),
                self.secret_store.clone(),
            ),
        ));
        let wa: crate::connections_api::SharedWhatsAppGateway = Arc::new(Mutex::new(
            daimonion_connect::whatsapp::WhatsAppGateway::new(self.screenpipe_dir.clone()),
        ));
        ...
        let mcp_store: crate::mcp_servers_api::SharedMcpServerStore =
            Arc::new(daimonion_connect::mcp_servers::McpServerStore::new(
                self.screenpipe_dir.clone(),
                self.secret_store.clone(),
            ));
