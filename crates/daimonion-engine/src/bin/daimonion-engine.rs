...
#[cfg(target_os = "macos")]
use daimonion_audio::core::device::{
...
use daimonion_core::find_ffmpeg_path;
use daimonion_core::paths;
...
use daimonion_screen::monitor::list_monitors;
use serde_json::json;
use std::{
    env, fs,
...
async fn doctor_check_system_audio_capture() -> bool {
    ...
    match get_cpal_device_and_config(&device).await {
        ...
        Err(e) => {
            let terminal = daimonion_core::permissions::detect_terminal();
...
fn setup_logging(
    ...
    let file_appender = daimonion_engine::logging::SizedRollingWriter::builder()
...
async fn main() -> anyhow::Result<()> {
    ...
    let record_args = match cli.command {
        ...
        Command::Connection { ref subcommand } => {
            daimonion_engine::cli::connection::handle_connection_command(subcommand).await?;
            return Ok(());
        }
        Command::Vault { ref subcommand } => {
            daimonion_engine::cli::vault::handle_vault_command(subcommand).await?;
            return Ok(());
        }
        Command::Install {
            ref url,
            ...
            daimonion_engine::cli::install::handle_install(url, allow_untrusted).await?;
            return Ok(());
        }
        Command::Login => {
            daimonion_engine::cli::login::handle_login_command().await?;
            return Ok(());
        }
        Command::Logout => {
            daimonion_engine::cli::login::handle_logout_command().await?;
            return Ok(());
        }
        Command::Whoami => {
            daimonion_engine::cli::login::handle_whoami_command().await?;
            return Ok(());
        }
        Command::Survey => {
            daimonion_engine::cli::survey::handle_survey_command().await?;
            return Ok(());
        }
        Command::Auth { ref subcommand } => {
            daimonion_engine::cli::auth::handle_auth_command(subcommand).await?;
            return Ok(());
        }
        Command::Db { ref subcommand } => {
            daimonion_engine::cli::db::handle_db_command(subcommand).await?;
            return Ok(());
        }
        Command::Backup {
            ref subcommand,
            ...
            daimonion_engine::cli::backup::handle_backup_command(subcommand, data_dir).await?;
            return Ok(());
        }
        Command::Export(ref args) => {
            daimonion_engine::cli::export::handle_export_command(args).await?;
            return Ok(());
        }
        Command::Doctor => {
            ...
            let perms_ok = daimonion_core::permissions::preflight_check(true, true);
    ...
    daimonion_connect::mdns::set_enabled(record_args.enable_mdns);
    ...
    tokio::spawn(async {
        daimonion_engine::cli_reminder::check_for_updates().await;
    });

    // Periodic terminal nudge to install the desktop app (CLI-only).
    daimonion_engine::cli_reminder::spawn();
    ...
    let _sentry_guard = if config.analytics_enabled {
        ...
        sentry::configure_scope(|scope| {
            ...
            let telemetry_context =
                daimonion_engine::telemetry_context::TelemetryContext::from_env();
    ...
    {
        use daimonion_core::permissions;
    ...
    let _permission_monitor_handle = daimonion_engine::permission_monitor::start();
    ...
    daimonion_engine::schedule_monitor::reset_schedule_paused();
        ...
        Ok(mut manager) => {
            ...
            if !config.disable_timeline {
                ...
                manager.set_on_transcription_insert(std::sync::Arc::new(move |info| {
                    ...
                    rt.spawn(async move {
                        use daimonion_engine::hot_frame_cache::HotAudio;
                        cache
                            .push_audio(HotAudio {
    ...
    let encryption_requested =
        config.encrypt_secrets || daimonion_secrets::is_encryption_requested(&local_data_dir);

    {
        ...
        let secret_key = if encryption_requested {
            if config.encrypt_secrets {
                match daimonion_secrets::keychain::get_or_create_key() {
                    Some(k) => {
                        info!("keychain: encryption key ready (--encrypt-secrets)");
                        if let Err(e) = daimonion_secrets::mark_encryption_enabled(&local_data_dir)
                        {
                ...
                match daimonion_secrets::keychain::get_key() {
                    daimonion_secrets::keychain::KeyResult::Found(k) => {
        ...
        let secret_store_result =
            daimonion_secrets::SecretStore::new(db.pool.clone(), secret_key).await;
        match secret_store_result {
            Ok(store) => {
                // Run startup permission sweep
                let fixed = daimonion_secrets::fix_secret_file_permissions(&local_data_dir);
                if fixed > 0 {
                    info!("fixed permissions on {} credential files", fixed);
                }

                // Run legacy migration
                match daimonion_secrets::migrate_legacy_secrets(&store, &local_data_dir).await {
                    Ok(report) => {
    ...
    if config.enable_workflow_events {
        ...
        tokio::spawn(async move {
            daimonion_engine::workflow_classifier::start_workflow_classifier(
                classifier_url,
                token,
                port,
    ...
    let mut agent_executors: std::collections::HashMap<
        String,
        std::sync::Arc<dyn daimonion_core::agents::AgentExecutor>,
    > = std::collections::HashMap::new();
    agent_executors.insert("pi".to_string(), pi_executor.clone());

    // Create pipe store backed by the main SQLite DB
    let pipe_store: Option<std::sync::Arc<dyn daimonion_core::pipes::PipeStore>> =
    ...
    pipe_manager.set_token_registry(std::sync::Arc::new(
        daimonion_engine::pipe_permissions_middleware::DashMapTokenRegistry::new(
        ...
        pipe_manager.set_connection_check(std::sync::Arc::new(move |required| {
            ...
            Box::pin(async move {
                ...
                for conn_id in required {
                    let configured = daimonion_connect::connections::is_connection_configured(
                        ss.as_deref(),
                        &dir,
                        &conn_id,
                    )
    ...
    println!(
        ...
        if record_args.retention_days == 0 {
            ...
            match record_args.retention_mode {
                daimonion_engine::retention::RetentionMode::Media => {
                    "media-only (keep transcripts)".to_string()
                }
                daimonion_engine::retention::RetentionMode::All => "all (full delete)".to_string(),
            }
        }
    );
    ...
    if config.async_image_pii_redaction {
        ...
        use daimonion_redact::{ImageRedactionPolicy, ImageRedactor};
        use std::sync::Arc;
