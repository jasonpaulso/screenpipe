...
use axum::Json;
use daimonion_connect::connections::render_context;
use daimonion_connect::mcp_servers::render_context as mcp_render_context;
use daimonion_core::pipes::PipeManager;
...
pub async fn list_favorites(State(pm): State<SharedPipeManager>) -> Json<Value> {
    ...
    let favorites = daimonion_core::pipes::favorites::load(&dir);
...
pub async fn set_pipe_favorite(
    ...
    let dir = mgr.pipes_dir().to_path_buf();
    drop(mgr);
    match daimonion_core::pipes::favorites::set(&dir, &id, body.favorite) {
