...
fn default_audio_engine() -> CliAudioTranscriptionEngine {
    let tier = daimonion_config::detect_tier();
    if matches!(tier, daimonion_config::DeviceTier::Low) {
...
pub struct RecordArgs {
    ...
    pub schedule_rules: Vec<daimonion_config::ScheduleRule>,
}
...
fn parse_schedule_rule(s: &str) -> Result<daimonion_config::ScheduleRule, String> {
    ...
    Ok(daimonion_config::ScheduleRule {
        day_of_week,
        start_time,
        end_time,
        record_mode,
...
impl RecordArgs {
    ...
    pub fn to_recording_settings(&self) -> daimonion_config::RecordingSettings {
        ...
        daimonion_config::RecordingSettings {
    ...
    pub async fn into_recording_config(
        ...
        let cli_encryption_ready = if self.encrypt_secrets {
            match daimonion_secrets::keychain::get_or_create_key() {
                Some(_) => {
                    daimonion_secrets::mark_encryption_enabled(&data_dir)?;
                    true
                }
                None => {
                    tracing::warn!(
        ...
        if settings.device_tier.is_none() {
            ...
            let existing = daimonion_config::load_toml(&config_path).ok();
            ...
            if has_tier {
                ...
                if is_fresh {
                    daimonion_config::apply_tier_defaults(&mut settings, tier);
        ...
        let tier = settings
            ...
            .unwrap_or_else(daimonion_config::detect_tier);
        if daimonion_config::is_engine_unsafe(&settings.audio_transcription_engine, tier) {
            let safe = daimonion_config::best_engine_for_platform(tier);
            eprintln!(
    ...
    fn apply_explicit_overrides(
        &self,
        settings: &mut daimonion_config::RecordingSettings,
        sources: &RecordArgSources,
    ) {
...
fn load_recording_settings_from_store(
    data_dir: &Path,
) -> anyhow::Result<Option<daimonion_config::RecordingSettings>> {
...
fn persist_recording_settings_to_store(
    data_dir: &Path,
    settings: &daimonion_config::RecordingSettings,
...
pub fn get_or_create_machine_id(override_id: Option<String>) -> String {
    ...
    daimonion_core::sync::get_or_create_machine_id()
}
...
mod tests {
    ...
    fn test_explicit_cli_overrides_preserve_app_settings_for_absent_defaults() {
        ...
        let mut settings = daimonion_config::RecordingSettings {
            port: 3030,
            disable_audio: false,
