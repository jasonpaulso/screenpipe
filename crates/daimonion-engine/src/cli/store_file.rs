...
pub fn store_path() -> PathBuf {
    daimonion_core::paths::default_screenpipe_data_dir().join("store.bin")
}
...
fn write_store_to_with_key(
    ...
    let bytes = if encrypt {
        ...
        let ciphertext = daimonion_vault::crypto::encrypt_small(&serialized, &key)
...
fn read_plain_store_bytes_with_key(
    ...
    let key = key_provider()?;
    daimonion_vault::crypto::decrypt_small(&data[STORE_MAGIC.len()..], &key)
...
fn store_encryption_key() -> Result<[u8; 32]> {
    match daimonion_secrets::keychain::get_key() {
        daimonion_secrets::keychain::KeyResult::Found(key) => Ok(key),
        daimonion_secrets::keychain::KeyResult::AccessDenied => bail!(
        ...
        daimonion_secrets::keychain::KeyResult::NotFound => bail!(
        ...
        daimonion_secrets::keychain::KeyResult::Unavailable => bail!(
...
mod tests {
    ...
    fn encrypted_store_bytes(value: &Value, key: &[u8; 32]) -> Vec<u8> {
        ...
        let ciphertext = daimonion_vault::crypto::encrypt_small(&plaintext, key).unwrap();
