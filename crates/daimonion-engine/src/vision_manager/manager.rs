...
use daimonion_screen::monitor::{get_monitor_by_id, list_monitors};
...
pub struct VisionManagerConfig {
    ...
    /// snapshot max width via `daimonion_core::video::*`. Values: "low",
...
impl VisionManager {
    ...
    pub fn is_monitor_allowed(&self, monitor: &daimonion_screen::monitor::SafeMonitor) -> bool {
        if self
    ...
    async fn start_event_driven_monitor(
        &self,
        monitor_id: u32,
        monitor: daimonion_screen::monitor::SafeMonitor,
    ) -> Result<JoinHandle<()>> {
        ...
        // single `videoQuality` setting via `daimonion_core::video` — that
        ...
        #[cfg(target_os = "macos")]
        daimonion_screen::monitor::set_sck_capture_max_width(max_snapshot_width);
