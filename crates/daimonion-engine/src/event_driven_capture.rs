...
use anyhow::Result;
use chrono::Utc;
use daimonion_a11y::tree::TreeWalkerConfig;
use daimonion_a11y::ActivityFeed;
use daimonion_capture::paired_capture::{paired_capture, CaptureContext, PairedCaptureResult};
use daimonion_core::window_pattern::{self, WindowPattern};
use daimonion_db::DatabaseManager;
use daimonion_screen::capture_screenshot_by_window::{get_excluded_sck_window_ids, WindowFilters};
use daimonion_screen::frame_comparison::{FrameComparer, FrameComparisonConfig};
use daimonion_screen::monitor::{list_monitors, SafeMonitor};
use daimonion_screen::snapshot_writer::SnapshotWriter;
use daimonion_screen::utils::capture_monitor_image;
...
pub struct CaptureParams<'a> {
    ...
    pub languages: &'a [daimonion_core::Language],
}
...
pub async fn event_driven_capture_loop(
    ...
    vision_metrics: Arc<daimonion_screen::PipelineMetrics>,
    hot_frame_cache: Option<Arc<HotFrameCache>>,
    use_pii_removal: bool,
    pause_on_drm_content: bool,
    languages: Vec<daimonion_core::Language>,
    ...
    let mut walk_budget = daimonion_a11y::budget::AppWalkBudget::new();
    ...
    loop {
        ...
        #[cfg(target_os = "macos")]
        if daimonion_screen::stream_invalidation::take() {
            info!(
...
fn resolve_capture_metadata(
    tree_snapshot: Option<&daimonion_a11y::tree::TreeSnapshot>,
    trigger: &CaptureTrigger,
...
async fn do_capture(
    ...
    walk_budget: &mut daimonion_a11y::budget::AppWalkBudget,
    ...
    use daimonion_a11y::tree::TreeWalkResult;
    if let Some(ref app) = trigger_app {
    ...
    let tree_walk_result = tokio::task::spawn_blocking(move || {
        daimonion_capture::paired_capture::walk_accessibility_tree(&config)
    })
    .await?;
