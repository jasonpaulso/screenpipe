...
use anyhow::Result;
use daimonion_a11y::{ExtractionThreadPriority, UiCaptureConfig, UiRecorder};
use daimonion_core::window_pattern::{self, WindowPattern};
use daimonion_db::{DatabaseManager, InsertUiEvent};
        ...
        loop {
            ...
            match handle.recv_timeout(recv_timeout) {
                Some(event) => {
                    ...
                    let is_scroll =
                        matches!(db_event.event_type, daimonion_db::UiEventType::Scroll);
                    let trigger_kind =
...
fn should_record_input_event(
    ...
    match db_event.event_type {
        daimonion_db::UiEventType::Key | daimonion_db::UiEventType::Text => {
            record_keyboard_events
        }
        daimonion_db::UiEventType::Clipboard => record_clipboard_events,
        _ => true,
    }
}
...
fn capture_trigger_kind(
    ...
    match &db_event.event_type {
        daimonion_db::UiEventType::AppSwitch => {
            if is_ignored {
                None
            } else {
        ...
        daimonion_db::UiEventType::WindowFocus => {
            if is_ignored {
                None
            } else {
        ...
        daimonion_db::UiEventType::Click => Some(
            target
        ...
        daimonion_db::UiEventType::Clipboard => Some(CaptureTrigger::Clipboard),
        ...
        daimonion_db::UiEventType::Text => Some(CaptureTrigger::TypingPause),
        ...
        daimonion_db::UiEventType::Scroll => None,
        // Key events fire a KeyPress trigger even when privacy settings
        // suppress storing the key row.
        daimonion_db::UiEventType::Key => Some(CaptureTrigger::KeyPress),
