...
use daimonion_db::{AudioChunkInfo, NewMeetingTranscriptSegment};
...
pub async fn retranscribe_handler(
    ...
    for chunk in &chunks {
        ...
        let (samples, sample_rate) =
            match daimonion_audio::utils::ffmpeg::read_audio_from_file(file_path) {
                Ok(result) => result,
                Err(e) => {
    ...
    for batch in batches {
        ...
        for chunk in batch {
            ...
            match tokio::task::spawn_blocking(move || {
                daimonion_audio::utils::ffmpeg::read_audio_from_file(StdPath::new(&path_owned))
            })
            .await
            {
                Ok(Ok((samples, sr))) => {
