...
async fn health_check_inner(state: &Arc<AppState>) -> HealthCheckResponse {
    ...
    for device in &audio_devices {
        ...
        let last_capture = daimonion_audio::core::get_device_capture_time(&device_name);
    ...
    if audio_devices.is_empty() {
        let last_capture = daimonion_audio::core::LAST_AUDIO_CAPTURE.load(Ordering::Relaxed);
...
) -> JsonResponse<daimonion_screen::MetricsSnapshot> {
...
pub(crate) async fn audio_metrics_handler(
    State(state): State<Arc<AppState>>,
) -> JsonResponse<daimonion_audio::metrics::AudioMetricsSnapshot> {
