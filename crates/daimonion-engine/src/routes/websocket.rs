...
use daimonion_events::{send_event, subscribe_to_all_events, Event as ScreenpipeEvent};
...
async fn handle_meeting_status_socket(
    ...
    let mut stream =
        daimonion_events::subscribe_to_event::<MeetingStatusResponse>("meeting_status_changed");
    loop {
        tokio::select! {
            event = stream.next() => {
