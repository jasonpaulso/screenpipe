...
use daimonion_db::{MeetingRecord, MeetingTranscriptSegment, MEETING_END_REASON_EXPLICIT_STOP};
...
pub fn emit_meeting_status_changed(status: &MeetingStatusResponse) {
    ...
    if let Err(e) = daimonion_events::send_event("meeting_status_changed", status.clone()) {
...
pub(crate) async fn stop_meeting_handler(
    ...
    // Emit event so triggered pipes can react
    if let Err(e) =
        daimonion_events::send_event("meeting_ended", serde_json::json!({ "meeting_id": id }))
    {
