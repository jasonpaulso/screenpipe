...
use daimonion_core::pii_removal::detect_pii_regions;
use daimonion_db::TextPosition;
        ...
        if let Ok(nodes) = serde_json::from_str::<Vec<serde_json::Value>>(&tree_json) {
            for n in &nodes {
                ...
                text_positions.push(TextPosition {
                    text: text.to_string(),
                    confidence: 1.0,
                    bounds: daimonion_db::TextBounds {
                        left,
                        top,
                        width,
                        height,
    ...
    if text_positions.is_empty() {
        if let Ok((_, Some(tree_json))) = state.db.get_frame_accessibility_data(frame_id).await {
            if let Some(query) = &params.query {
                ...
                text_positions = daimonion_db::find_matching_a11y_positions(&tree_json, query);
...
pub async fn run_frame_ocr(
    ...
    let ocr_result = tokio::task::spawn_blocking(move || {
        let (text, json, _confidence) = daimonion_screen::perform_ocr_apple(&image, &[]);
        (text, json)
    })
    .await
    ...
    let ocr_result = match daimonion_screen::perform_ocr_windows(&image, &[]).await {
    ...
    let ocr_result = tokio::task::spawn_blocking(move || {
        let (text, json, _confidence) = daimonion_screen::perform_ocr_tesseract(&image, vec![]);
        (text, json)
    })
    .await
    .unwrap_or_else(|_| (String::new(), "[]".to_string()));
    ...
    if !ocr_text.is_empty() {
        #[cfg(target_os = "macos")]
        let engine = Arc::new(daimonion_db::OcrEngine::AppleNative);
        #[cfg(target_os = "windows")]
        let engine = Arc::new(daimonion_db::OcrEngine::WindowsNative);
        #[cfg(not(any(target_os = "macos", target_os = "windows")))]
        let engine = Arc::new(daimonion_db::OcrEngine::Tesseract);
        if let Err(e) = state
            .db
