...
use daimonion_db::{DatabaseManager, MEETING_END_REASON_AUTO_END, MEETING_END_REASON_SHUTDOWN};
use daimonion_events::subscribe_to_event;
...
/// Kept in sync with `daimonion_audio`'s `AUDIO_RECENCY_WINDOW_MS`.
...
pub async fn run_meeting_detection_loop(
    ...
    detector: Option<Arc<daimonion_audio::meeting_detector::MeetingDetector>>,
            ...
            if manual_matches || detector_matches {
                    ...
                    .await
                {
                    Ok(()) => {
                        ...
                        if let Err(e) = daimonion_events::send_event(
                            "meeting_ended",
