...
pub fn set_screen_locked(locked: bool) {
    ...
    daimonion_config::set_screen_locked(locked);
}
...
pub fn start_sleep_monitor() {
    ...
    if initial_locked {
        ...
        daimonion_config::set_screen_locked(true);
    }
    ...
    std::thread::spawn(|| {
        ...
        unsafe extern "C" fn on_screen_unlocked(
            ...
            daimonion_config::set_screen_locked(false);
            if was_locked {
                ...
                #[cfg(target_os = "macos")]
                daimonion_screen::stream_invalidation::request();
    ...
    std::thread::spawn(|| {
        ...
        unsafe extern "C" fn on_display_reconfigured(
            ...
            #[cfg(target_os = "macos")]
            daimonion_screen::stream_invalidation::request();
            daimonion_audio::stream_invalidation::request();
...
fn on_will_sleep() {
    ...
    daimonion_db::request_write_pause();
...
fn on_did_wake(handle: &tokio::runtime::Handle) {
    ...
    handle.spawn(async {
        ...
        daimonion_db::request_write_resume();
...
mod tests {
    ...
    async fn test_on_did_wake_sets_audio_invalidation() {
        // Clear stale flags
        let _ = daimonion_audio::stream_invalidation::take();
        let _ = daimonion_screen::stream_invalidation::take();
        RECENTLY_WOKE.store(false, Ordering::SeqCst);
        ...
        assert!(
            daimonion_audio::stream_invalidation::take(),
            "Audio stream invalidation flag must be set after wake"
        );
        assert!(
            daimonion_screen::stream_invalidation::take(),
