...
async fn open_secret_store(data_dir: &Path) -> Result<daimonion_secrets::SecretStore> {
    ...
    let key = if daimonion_secrets::is_encryption_requested(data_dir) {
        match daimonion_secrets::keychain::get_key() {
            daimonion_secrets::keychain::KeyResult::Found(k) => Some(k),
            _ => None,
        }
    } else {
        None
    };
    let store = daimonion_secrets::SecretStore::new(pool, key).await?;
    Ok(store)
}
