...
use daimonion_engine::video_utils::extract_frames_from_video;
use daimonion_screen::capture_screenshot_by_window::CapturedWindow;
#[cfg(target_os = "macos")]
use daimonion_screen::perform_ocr_apple;
...
async fn test_get_video_metadata() -> Result<()> {
    ...
    let metadata =
        daimonion_engine::video_utils::get_video_metadata(video_path.to_str().unwrap()).await?;
