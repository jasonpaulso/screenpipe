...
use regex::Regex;
use daimonion_a11y::tree::{create_tree_walker, TreeSnapshot, TreeWalkerConfig};
use daimonion_core::pii_removal::remove_pii;
...
pub struct CaptureContext<'a> {
    ...
    pub languages: Vec<daimonion_core::Language>,
...
pub async fn paired_capture(
    ...
    let (ocr_text, ocr_text_json) = if !has_accessibility_text || a11y_is_thin {
        ...
        let raw = {
            match daimonion_screen::perform_ocr_windows(&ctx.image, &ctx.languages).await {
            ...
            tokio::task::spawn_blocking(move || {
                #[cfg(target_os = "macos")]
                {
                    let (text, json, _confidence) =
                        daimonion_screen::perform_ocr_apple(&image_for_ocr, &languages);
                    (text, json)
                }
                #[cfg(not(target_os = "macos"))]
                {
                    let (text, json, _confidence) =
                        daimonion_screen::perform_ocr_tesseract(&image_for_ocr, languages);
                    (text, json)
                }
            })
            .await
...
pub fn walk_accessibility_tree(config: &TreeWalkerConfig) -> daimonion_a11y::tree::TreeWalkResult {
...
fn a11y_content_is_thin(
    snap: &daimonion_a11y::tree::TreeSnapshot,
...
fn sanitize_ocr_text_json(text_json: &str) -> String {
    ...
    let sanitized = daimonion_core::pii_removal::remove_pii_from_text_json(&entries);
...
mod tests {
    ...
    use daimonion_a11y::tree::AccessibilityTreeNode;
    ...
    async fn test_paired_capture_with_accessibility_text() {
        ...
        let snap = TreeSnapshot {
            ...
            simhash: 67890,
            truncated: false,
            truncation_reason: daimonion_a11y::tree::TruncationReason::None,
            max_depth_reached: 0,
        };
