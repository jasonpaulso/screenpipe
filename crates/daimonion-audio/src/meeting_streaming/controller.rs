...
pub fn start_meeting_streaming_loop(
    ...
    tokio::spawn(async move {
        ...
        let mut started_sub =
            daimonion_events::subscribe_to_event::<MeetingLifecycleEvent>("meeting_started");
        let mut ended_sub =
            daimonion_events::subscribe_to_event::<MeetingLifecycleEvent>("meeting_ended");
        let mut delta_sub = daimonion_events::subscribe_to_event::<MeetingTranscriptDelta>(
            "meeting_transcript_delta",
        );
        let mut final_sub = daimonion_events::subscribe_to_event::<MeetingTranscriptFinal>(
            "meeting_transcript_final",
        );
        let mut error_sub = daimonion_events::subscribe_to_event::<MeetingStreamingError>(
            ...
            tokio::select! {
                ...
                _ = inactivity_tick.tick() => {
                        ...
                        .and_then(|session| auto_end_reason(session, Instant::now()))
                    {
                        ...
                        let _ = daimonion_events::send_event(
...
async fn start_streaming_session(
    ...
    *active = Some(ActiveMeetingStream {
    ...
    let _ = daimonion_events::send_event("meeting_streaming_session_started", started);
}
...
fn emit_session_ended(session: ActiveMeetingStream) {
    ...
    let _ = daimonion_events::send_event("meeting_streaming_session_ended", ended);
}
...
fn emit_status(
    ...
    let _ = daimonion_events::send_event("meeting_streaming_status_changed", status);
}
...
fn emit_error(
    ...
    let _ = daimonion_events::send_event("meeting_streaming_error", event);
}
...
mod tests {
    ...
    // `check_and_emit_stall_notifications` calls `daimonion_events::send_event`,
