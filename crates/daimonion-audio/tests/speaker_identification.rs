mod tests {
    ...
    use daimonion_audio::speaker::segment::get_segments;
    ...
    async fn test_audio_speaker_identification() {
        ...
        let (mut multiple_speakers_data, multiple_speakers_sample_rate) =
            daimonion_audio::pcm_decode(&multiple_speakers_path)
                .expect("Failed to decode audio file");

        // reesample if not 16000
        if multiple_speakers_sample_rate != 16000 {
            multiple_speakers_data = daimonion_audio::resample(
        ...
        let (mut obama_data, obama_sample_rate) =
            daimonion_audio::pcm_decode(&obama_path).expect("Failed to decode audio file");

        // reesample if not 16000
        if obama_sample_rate != 16000 {
            obama_data = daimonion_audio::resample(&obama_data, obama_sample_rate, 16000).unwrap();
        }
