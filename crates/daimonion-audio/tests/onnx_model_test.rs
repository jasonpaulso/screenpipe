...
mod tests {
    ...
    use daimonion_audio::speaker::models::{get_or_download_model, PyannoteModel};
    use daimonion_audio::vad::silero::SileroVad;
    use daimonion_audio::vad::VadEngine;
    ...
    async fn test_onnx_segmentation_model_loads() -> Result<()> {
        ...
        let start = Instant::now();
        let session = daimonion_audio::speaker::create_session(&segmentation_model.path)?;
    ...
    async fn test_onnx_startup_sequence() -> Result<()> {
        ...
        let _segmentation_session =
            daimonion_audio::speaker::create_session(&segmentation_model.path)?;
