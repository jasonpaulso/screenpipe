...
//! RF-DETR-Nano image-PII detector via [`daimonion_rfdetr_mlx`] —
...
///
/// Mirrors `daimonion_config::PARAKEET_MIN_MACOS_MAJOR` — mlx-sys's
...
/// `daimonion_config::macos_major_version`, inlined here so this
...
mod imp {
    ...
    type ModelHandle = Arc<StdMutex<daimonion_rfdetr_mlx::RfDetr>>;
        ...
        async fn ensure_loaded(&self) -> Result<ModelHandle, RedactError> {
            ...
            if state.model.is_none() {
                ...
                let model = tokio::task::block_in_place(|| {
                    let mut m = daimonion_rfdetr_mlx::RfDetr::load(&path)
