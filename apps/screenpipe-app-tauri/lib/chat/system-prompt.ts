...
export function buildConnectionsContext(
  ...
  const entries = withDesc
    .map((c) => `## ${c.name} (${c.id})\n${c.description}`)
    .join("\n\n");
  return `\n\n# Connected integrations\n\nThe user has connected the following external services. Use the endpoints listed under each to fetch live data when relevant. All endpoints are on http://localhost:3030 and require \`-H "Authorization: Bearer $DAIMONION_LOCAL_API_KEY"\`.\n\n${entries}`;
}
