...
# NEXT_PUBLIC_DAIMONION_E2E bypasses the billing gate by default so the suite
# exercises real features; the dedicated entitlement-gate spec re-enables it.
NEXT_PUBLIC_DAIMONION_E2E=true bun tauri build --no-sign --debug --verbose --no-bundle -- --features e2e
echo "Running E2E..."
bun run test:e2e
