...
Query the user's screen history via their screenpipe REST API at http://DAIMONION_IP:3030.

## search content

```bash
curl -s "http://DAIMONION_IP:3030/search?q=QUERY&limit=20"
```
...
curl -s "http://DAIMONION_IP:3030/search?q=QUERY&content_type=all"

# audio transcriptions
curl -s "http://DAIMONION_IP:3030/search?q=QUERY&content_type=audio"
```

## activity summary

```bash
curl -s "http://DAIMONION_IP:3030/activity-summary?start_time=2024-01-15T10:00:00Z&end_time=2024-01-15T18:00:00Z"
```

## list meetings

```bash
curl -s "http://DAIMONION_IP:3030/meetings?limit=20"
```
````

Replace `DAIMONION_IP` with the IP of the machine running screenpipe. If the machines aren't on the same network, use [Tailscale](https://tailscale.com) to create a private network between them.
...
Or set env vars instead of flags: `DAIMONION_REMOTE_HOST`, `DAIMONION_REMOTE_USER`, `DAIMONION_REMOTE_KEY`, `DAIMONION_REMOTE_PATH`.
...
The MCP server also provides **resources** (`daimonion://context` for current time, `daimonion://guide` for search strategy) and **prompts** (search-recent, find-in-app, meeting-notes).
