...
| "unauthorized" | API auth or provider key missing | set `DAIMONION_API_KEY`, sign in, or update provider credentials |
...
- use `DAIMONION_API_KEY` when API auth is enabled
