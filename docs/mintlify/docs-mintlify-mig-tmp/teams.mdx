...
<Steps>
  ...
  <Step title="admin shares invite link">
    the invite link contains the team ID and the base64-encoded encryption key: `daimonion://join-team?team_id=...&key=...`. this is the only time the key is transmitted — via the link itself, not through our server.
  </Step>
