...
if API auth is enabled, add `-H "Authorization: Bearer $DAIMONION_API_KEY"`. see [API recipes](/api-recipes) for meetings, speakers, frames, memories, retention, archive, and delete-range examples.
