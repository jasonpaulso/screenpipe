...
to troubleshoot issues, enable debug logging by setting the `DAIMONION_LOG` environment variable before starting screenpipe:

**macOS/Linux:**
```bash
DAIMONION_LOG=debug screenpipe
```

**Windows (PowerShell):**
```powershell
$env:DAIMONION_LOG = "debug"
screenpipe
```
...
```bash
DAIMONION_LOG=screenpipe=debug,vision=debug screenpipe
```
