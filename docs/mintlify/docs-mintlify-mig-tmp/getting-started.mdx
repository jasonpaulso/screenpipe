...
```bash
DAIMONION_NO_REMINDERS=1 npx screenpipe@latest record
```

or set it once in your shell:
```bash
export DAIMONION_NO_REMINDERS=1
npx screenpipe@latest record
```
...
```bash
export DAIMONION_API_KEY=$(screenpipe auth token)
curl "http://localhost:3030/search?q=example" \
  -H "Authorization: Bearer $DAIMONION_API_KEY"
```
