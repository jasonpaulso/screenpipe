...
<Tip>
if API auth is enabled, add `-H "Authorization: Bearer $DAIMONION_API_KEY"` to requests. get the key with `screenpipe auth token` or from settings -> privacy -> API security.
</Tip>
