...
- **`daimonion://view` links open in the in-app viewer everywhere** — deep links inside notifications, notification history, chat, and the viewer itself now all route through the same handler, so a link copied from a notification into chat opens the in-app viewer instead of falling through to the browser.
